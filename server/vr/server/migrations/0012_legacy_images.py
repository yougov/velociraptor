...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Create a stack named 'legacy'.
        legacy_stack = orm['server.OSStack'](
            name='legacy',
            desc='Created by data migration',
        )
        legacy_stack.save()

        # Link all existing os images to this stack.
        for img in orm['server.OSImage'].objects.all():
            img.stack = legacy_stack
            img.active = True
            img.save()

        # Find any builds that have os images linked to them.  Link all their
        # apps to the legacy stack.  This will ensure that Swarming on these
        # apps will do the right thing on the next run after this migration
        # happens.
        for build in orm['server.Build'].objects.filter(os_image__isnull=False):
            app = build.app
            app.stack = legacy_stack
            app.save()
    ...
    def backwards(self, orm):
        ...
        for app in orm['server.App'].objects.all():
            app.stack = None
            app.save

        for stack in orm['server.OSStack'].objects.filter(name='legacy'):
            stack.delete()
