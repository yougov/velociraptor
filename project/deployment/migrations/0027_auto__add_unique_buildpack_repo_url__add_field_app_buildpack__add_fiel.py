...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'BuildPack', fields ['repo_url']
        db.create_unique('deployment_buildpack', ['repo_url'])

        # Adding field 'App.buildpack'
        db.add_column('deployment_app', 'buildpack',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['deployment.BuildPack'], null=True, blank=True),
                      keep_default=False)

        # Adding field 'Build.buildpack_url'
        db.add_column('deployment_build', 'buildpack_url',
                      self.gf('django.db.models.fields.CharField')(default='This build predates buildpack support', max_length=200),
                      keep_default=False)

        # Adding field 'Build.buildpack_revision'
        db.add_column('deployment_build', 'buildpack_revision',
                      self.gf('django.db.models.fields.CharField')(default='na', max_length=50),
                      keep_default=False)


    def backwards(self, orm):
        # Removing unique constraint on 'BuildPack', fields ['repo_url']
        db.delete_unique('deployment_buildpack', ['repo_url'])

        # Deleting field 'App.buildpack'
        db.delete_column('deployment_app', 'buildpack_id')

        # Deleting field 'Build.buildpack_url'
        db.delete_column('deployment_build', 'buildpack_url')

        # Deleting field 'Build.buildpack_revision'
        db.delete_column('deployment_build', 'buildpack_revision')
    ...
    models = {
        ...
        'deployment.swarm': {
            'Meta': {'ordering': "['recipe__app__name', 'recipe__name', 'proc_name']", 'unique_together': "(('recipe', 'squad', 'proc_name'),)", 'object_name': 'Swarm'},
            'active': ('django.db.models.fields.BooleanField', [], {'default': 'True'}),
